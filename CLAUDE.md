...
7. **Web analytics** — research + add a web-analytics solution (Vercel Analytics / Plausible / Umami / GA4). Choose per privacy + cost + simplicity; Vercel Analytics is ~one line on the current stack.
8. **Google Search Console + speed up indexing** — the site is new, so Google must discover → crawl → index. Steps: (a) GSC → Add property → **Domain** `davidefantauzzi.me`, verify via a **Cloudflare DNS TXT** record (`google-site-verification=…`, DNS-only); (b) **submit the sitemap** `sitemap.xml` (now correct — prod URLs + hreflang); (c) **URL Inspection → Request indexing** for `/en`, `/it`, and `/` (limited requests/day → key pages only); (d) confirm no `noindex` header/meta and no `Disallow: /` on public pages (robots.txt already correct). Realistic timeline: homepage days–1wk, full site 1–4wk; the brand query "Davide Fantauzzi" indexes fastest. No paid shortcut. Optional boosts: a few inbound links (LinkedIn/GitHub profile → site), and `metadataBase`/OG already set via `generateMetadata`.
