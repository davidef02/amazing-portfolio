...
- [x] **Definizione Token `@theme`:** Verificare che `src/app/(frontend)/globals.css` sia configurato correttamente per mappare le classi utili di Neobrutal UI Live (`rounded-base`, `bg-main`, `shadow-brutal`, `bg-hot-pink`, `bg-mint`, `bg-lemon`) e le utility di focus/transizione.

## 2. Creazione Modelli nel Database (Payload CMS)
- [ ] **Collezione `Skills` (`src/collections/Skills/index.ts`):**
  - Definisci lo slug `skills`.
  - Campo `name` (text, required: true).
  - Campo `badgeColor` (select con opzioni: yellow, pink, cyan, green).
- [ ] **Collezione `Projects` (`src/collections/Projects/index.ts`):**
...
- [ ] **Global `SiteConfig` (`src/globals/SiteConfig/index.ts`):**
  - Crea un unico file di impostazioni globali strutturato a schede (tabs) per tenere pulita la dashboard:
    - **Tab 1: Hero**
...
- [ ] **Registrazione e Generazione Tipi:**
  - Importa e aggiungi `Skills`, `Projects` e `SiteConfig` in `src/payload.config.ts`.
  - Esegui `npm run generate:types` nel terminale per rigenerare il file `payload-types.ts`.
...
- [ ] **Compilazione:**
  ...
  - Compilare la scheda unica `SiteConfig` (Hero, Header e Footer).
...
- [ ] **Data Fetching (`src/app/(frontend)/page.tsx`):**
  ...
  - Usare `getPayload` per estrarre in modo asincrono i dati dei progetti e il globale `SiteConfig`.
- [ ] **Sviluppo Hero & Layout:**
  - Stampare il titolo e la bio dal blocco Hero di `SiteConfig`.
  - Renderizzare l'header e il footer prelevando i dati da `SiteConfig`.
